...
export const ForgotPassword = () => {
    ...
    const form = useForm({
        ...
        onSubmit: async ({ value }) => {
            try {
                const response = await fetch(`${import.meta.env.VITE_SERVER_URL}/api/v1/user/sendPasswordResetMail`, {
                    method: "POST",
                    headers: {
    ...
    return (
        <div className="w-full h-screen bg-black flex items-center justify-center p-4">
            ...
            <div className="w-full max-w-xl p-10 rounded-3xl [box-shadow:inset_0px_0px_10px_#aaa] bg-neutral-800/50 backdrop-blur-sm text-white flex flex-col items-center justify-center gap-4 relative">
                    ...
                    <form.Field
                        ...
                        children={(field) => {
                            return (
                                <>
                                    <div className="w-full flex flex-col gap-1">
                                        <label htmlFor={field.name} className="px-2 mb-1">Confirm Email:</label>
                                        <input
                                            id={field.name}
                                            name={field.name}
                                            value={field.state.value}
                ...
                <div>
                    <p className="text-sm italic text-neutral-500">Go back to <Link to={"/signin"} className="text-blue-500 underline not-italic pl-1">Sign in</Link>.</p>
                </div>
            </div>
        </div>
    )
