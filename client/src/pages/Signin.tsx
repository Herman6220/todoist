...
const Signin = () => {
    ...
    const form = useForm({
        ...
        onSubmit: async ({ value }) => {
            try {
                console.log(value);
                const response = await fetch(`${import.meta.env.VITE_SERVER_URL}/api/v1/user/auth/signin`, {
                    method: "POST",
                    headers: {
                ...
                if (!response.ok) {
                    console.log(`Error while signing in. Status: ${response.status}`);
                    return;
                }
        ...
        <div className="w-full h-screen bg-black flex items-center justify-center">
            ...
            <div className="w-full max-w-xl p-10 rounded-3xl [box-shadow:inset_0px_0px_10px_#aaa] bg-neutral-800/50 backdrop-blur-sm text-white flex flex-col items-center justify-center gap-4 relative">
                ...
                <div className="flex flex-col gap-2">
                    <p className="text-sm italic text-neutral-500">Not registered? <Link to={"/signup"} className="text-blue-500 underline not-italic pl-1">Sign up</Link> Now</p>
                    <Link to={"/forgot-password"} className="text-center font-light text-sm underline text-neutral-200">Forgot password</Link>
                </div>
            </div>
        </div>
    )
